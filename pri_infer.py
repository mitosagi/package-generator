...
def pri_infer(info_prompt, isAPI=True):
    ...
    return '\n'.join([
        ...
        'ここまでの情報からjson文字列を生成、validate_by_json_schemaにより検証してください。' if isAPI else 'これらの情報からjsonファイルを生成しjson schemaであるschema.jsonによりエラーを出力して検証を行ってください。エラーのある場合はリトライしてください。その後、完成したjsonファイルのダウンロードリンクを提供してください。もし、json.dump()を使用する場合は ensure_ascii=Falseを指定してください。'])
